...
app.listen(PORT, async () => {
    console.log('\n' + '='.repeat(60));
    console.log('🎨 AI图片生成服务器已启动（通义万相 + R2存储�?);
    console.log('='.repeat(60));
    console.log(`📡 服务地址: http://localhost:${PORT}`);
    console.log(`🔧 文字生成: http://localhost:${PORT}/api/tongyi/generate`);
    console.log(`📧 联系表单: http://localhost:${PORT}/api/contact`);
    console.log(`💚 健康检�? http://localhost:${PORT}/health`);
    console.log('='.repeat(60));
    console.log('☁️  Cloudflare R2 存储状�?');
    console.log(`   存储�? ${R2_BUCKET_NAME}`);
    console.log(`   端点: ${R2_ENDPOINT}`);
    console.log(`   公开URL: ${R2_PUBLIC_URL}`);
    console.log(`   🔍 测试R2连接...`);
    
    if (R2_ENABLED) {
        console.log(`   �?R2连接成功 - 图片将永久存储到R2`);
    } else {
        console.log(`   ⚠️  R2未配�?- 将使用临时URL`);
    }
